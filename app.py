...
client = MongoClient('mongodb://bum:bum@52.79.133.185', 27017) # 추후 서버에 맞춰야함
...
# 회원가입
# 토큰 암호화, 복호화를 위한 키
SECRET_KEY = '2022JUNGLE' #물론 이렇게 짜면 바로 들킴. 예시니까.
...
def comment_post():
    ...
    try:
        ...
        # print(worldcupid_receive, winner_receive)
        ...
        print(cupresult1['name'], cupresult2['title'])
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms='HS256')
        user_info = db.Users.find_one({"id": payload["id"]})
        print(user_info)
